...
if [ ! -d "$NATIVE_PROJECT/module/src" ] || [ ! -d "$NATIVE_PROJECT/module/include" ]; then
...
cp "$NATIVE_PROJECT/module/src/"*.c "$NATIVE_PROJECT/module/src/"*.S "$TARGET_SOURCE_DIR/"
for patch_file in "$PATCH_DIR"/*.patch; do
    patch -s -d "$TARGET_SOURCE_DIR" -p1 < "$patch_file"
done

"$CC_BIN" -std=c11 -O1 -g -Wall -Wextra -Werror -Wno-unused-function -fsanitize=address,undefined -fno-omit-frame-pointer -I "$SCRIPT_DIR/include" -I "$NATIVE_PROJECT/module/include" "$TARGET_SOURCE_DIR/native_fs.c" "$SCRIPT_DIR/native_fs_host_test.c" -o "$WORK_DIR/native_fs_host_test"
"$WORK_DIR/native_fs_host_test"
